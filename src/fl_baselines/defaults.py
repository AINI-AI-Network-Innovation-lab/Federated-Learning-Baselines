...
def register_default_components() -> None:
    ...
    from fl_baselines.algorithms.fedadmm import FedADMMBuilder
    ...
    from fl_baselines.algorithms.fedsikd import FedSiKDBuilder
    from fl_baselines.algorithms.fedproto import FedProtoBuilder
    from fl_baselines.algorithms.fedlama import FedLAMABuilder
    ...
    from fl_baselines.algorithms.fedamp import FedAMPBuilder
    from fl_baselines.algorithms.fedlaa import FedLAABuilder
    ...
    if "fedadmm" not in ALGORITHMS:
        ALGORITHMS.register("fedadmm", FedADMMBuilder())
    ...
    if "fedsikd" not in ALGORITHMS:
        ALGORITHMS.register("fedsikd", FedSiKDBuilder())
    if "fedrs" not in ALGORITHMS:
        ALGORITHMS.register("fedrs", FedRSBuilder())
    if "fedlama" not in ALGORITHMS:
        ALGORITHMS.register("fedlama", FedLAMABuilder())
    ...
    if "fedala" not in ALGORITHMS:
        ALGORITHMS.register("fedala", FedALABuilder())
    if "fedamp" not in ALGORITHMS:
        ALGORITHMS.register("fedamp", FedAMPBuilder())
    if "fedlaa" not in ALGORITHMS:
        ALGORITHMS.register("fedlaa", FedLAABuilder())
