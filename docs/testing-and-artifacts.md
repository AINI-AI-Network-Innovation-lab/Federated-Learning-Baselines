...
- `FedSiKD` không persist state client riêng; server giữ cluster-teacher snapshot trong memory và chỉ ghi checkpoint round/model như các FedAvg-style baseline
...
- `fedadmm_clients/<client-id>/state.pt`: versioned FedADMM primal `x_i`, dual `z_i`, and transformed `hat_x_i` state
- `fedadmm_server/state.pt`: FedADMM server `tilde_x`, consensus `bar_x`, and per-client `hat_x_i` cache
